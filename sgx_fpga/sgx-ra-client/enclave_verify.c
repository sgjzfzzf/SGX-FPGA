...
int verify_enclave_identity(sgx_measurement_t req_mr_signer,
                            ...
                            sgx_isv_svn_t min_isvsvn, int allow_debug,
                            sgx_report_body_t *report) {
  if (verbose) {
    edividerWithText("Client enclave Identity");
    eprintf(
        "Enclave MRSIGNER      = %s\n",
        hexstring((const char *)&report->mr_signer, sizeof(sgx_measurement_t)));
  ...
  if (memcmp((const void *)&report->mr_signer, (const void *)&req_mr_signer,
             sizeof(sgx_measurement_t))) {
