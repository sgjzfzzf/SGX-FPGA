...
int main(int argc, char *argv[]) {
  ...
  if (result) {
    ...
    printf("The transmission time is: %ld\n",
           tv_4.tv_sec * 1000 * 1000 + tv_4.tv_nsec / 1000 -
               (tv_3.tv_sec * 1000 * 1000 + tv_3.tv_nsec / 1000));
#endif
  }
  delete msgio;
...
pFile = fopen("ra_client.txt", "a");
  fprintf(pFile, "%ld\t%ld\n", (tv_2.tv_sec*1000*1000 + tv_2.tv_nsec/1000) -
(tv_1.tv_sec*1000*1000 + tv_1.tv_nsec/1000), (tv_4.tv_sec*1000*1000 +
tv_4.tv_nsec/1000) - (tv_3.tv_sec*1000*1000 + tv_3.tv_nsec/1000));
  fclose(pFile);
  */
  // timespec tv_2;
  // clock_gettime(CLOCK_MONOTONIC,&tv_2);
  // printf("The attestation time is: %ld\n", tv_2.tv_sec*1000*1000 +
  // tv_2.tv_nsec/1000 - (tv_1.tv_sec*1000*1000 + tv_1.tv_nsec/1000));
...
int do_attestation(sgx_enclave_id_t eid, config_t *config) {
  ...
  if (retPibCmp == 0) {
    ...
    if (debug) {
      ...
      print_hexstring(stderr, &msg4->platformInfoBlob,
                      sizeof(sgx_platform_info_t));
      // print_hexstring(fplog, &msg4->platformInfoBlob, sizeof
      // (sgx_platform_info_t));
      eprintf("\n");
    }
  ...
  if ( enclaveTrusted == Trusted ) {
          ...
          if ( debug ) eprintf("+++ ECALL enclage_ra_get_key_hash (MK) ret=
  0x%04x\n", status);
  ...
  0x%04x\n", status);
          ...
          if ( debug ) eprintf("+++ sgx_ra_get_keys (MK) ret= 0x%04x\n",
  key_status); if ( verbose ) { eprintf("SHA256(MK) = ");
...
int data_transmission_trusted_path(size_t data_size, sgx_enclave_id_t eid) {
  ...
  while (client_sockfd_adapter == -1) {
    client_sockfd_adapter =
        accept(server_sockfd_adapter, (struct sockaddr *)&client_addr_adapter,
               &client_adapter_len);
  }
  ...
  clock_gettime(CLOCK_MONOTONIC, &tv_1);
#endif
  status = generate_encrypted_message(eid, &cipher_rv, plain_text, data_size,
                                      encrypted_text, enc_data_size, flag);
  ...
  // (tv_1.tv_sec*1000*1000 + tv_1.tv_nsec/1000)); eprintf("ecall return value
  // #1: %ld\n", status); eprintf("ecall return value #2: %ld\n", cipher_rv);
  ...
  send(client_sockfd_adapter, encrypted_text, sizeof(uint8_t) * enc_data_size,
       0);
  ...
  // send_encrypetd_message_to_enclave(eid, &cipher_rv, encrypted_message,
  // true_receive_size);
#ifdef Time_Eval
  timespec tv_2;
  clock_gettime(CLOCK_MONOTONIC, &tv_2);
  ...
  // fprintf(pFile, "%ld\t%ld\n", (tv1.tv_sec*1000*1000 +
  // tv1.tv_nsec/1000)-(tv_1.tv_sec*1000*1000 + tv_1.tv_nsec/1000),
  // (tv_2.tv_sec*1000*1000 + tv_2.tv_nsec/1000) - (tv2.tv_sec*1000*1000 +
  // tv2.tv_nsec/1000)); fclose(pFile);
  return 0;
};
...
int process_data_ocall(size_t data_size, sgx_enclave_id_t eid) {
  ...
  */
  ...
  uint8_t *plain_message =
      (uint8_t *)malloc(sizeof(uint8_t) * data_receive_size);
  memcpy(plain_message, buffer, data_receive_size);
  send_plain_message_to_enclave(eid, &cipher_rv, plain_message,
                                data_receive_size);
  timespec tv_2;
  clock_gettime(CLOCK_MONOTONIC, &tv_2);
  FILE *pFile;
