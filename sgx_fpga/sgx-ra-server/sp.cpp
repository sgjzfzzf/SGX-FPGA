...
int get_attestation_report(IAS_Connection *ias, int version,
                           const char *b64quote,
                           sgx_ps_sec_prop_desc_t sec_prop, ra_msg4_t *msg4,
                           int strict_trust);
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    c = getopt_long(argc, argv, "A:B:DGI:J:K:N:PR:S:V:X:dg:hk:lp:r:s:i:j:vxz",
                    long_opt, &opt_index);
    if (c == -1)
      break;
    ...
    case 'I':
      ...
      if (!from_file((unsigned char *)&config.pri_subscription_key, optarg,
                     &offset)) {
        eprintf("IAS Primary Subscription Key must be %d-byte hex string.\n",
                IAS_SUBSCRIPTION_KEY_SIZE);
        return 1;
    ...
    case 'J':
      ...
      // Read the contents of the file
      if (!from_file((unsigned char *)&config.sec_subscription_key, optarg,
                     &offset)) {
        eprintf("IAS Secondary Subscription Key must be %d-byte hex string.\n",
                IAS_SUBSCRIPTION_KEY_SIZE);
        return 1;
    ...
    case 'i':
      ...
      strncpy((char *)config.pri_subscription_key, optarg,
              IAS_SUBSCRIPTION_KEY_SIZE);
    ...
    case 'j':
      ...
      strncpy((char *)config.sec_subscription_key, optarg,
              IAS_SUBSCRIPTION_KEY_SIZE);
  ...
  if (debug) {
    eprintf("+++ IAS Primary Subscription Key set to "
            "'%c%c%c%c........................%c%c%c%c'\n",
            config.pri_subscription_key[0], config.pri_subscription_key[1],
            config.pri_subscription_key[2], config.pri_subscription_key[3],
    ...
    eprintf("+++ IAS Secondary Subscription Key set to "
            "'%c%c%c%c........................%c%c%c%c'\n",
            config.sec_subscription_key[0], config.sec_subscription_key[1],
            config.sec_subscription_key[2], config.sec_subscription_key[3],
  ...
  if (config.service_private_key == NULL) {
    ...
    config.service_private_key =
        key_private_from_bytes(def_service_private_key);
    if (config.service_private_key == NULL) {
  ...
  if (debug) {
    ...
    PEM_write_PrivateKey(stderr, config.service_private_key, NULL, NULL, 0, 0,
                         NULL);
    PEM_write_PrivateKey(fplog, config.service_private_key, NULL, NULL, 0, 0,
                         NULL);
  }
  ...
  try {
    ias = new IAS_Connection((flag_prod) ? IAS_SERVER_PRODUCTION
                                         : IAS_SERVER_DEVELOPMENT,
                             0, (char *)(config.pri_subscription_key),
...
int process_msg3(MsgIO *msgio, IAS_Connection *ias, sgx_ra_msg1_t *msg1,
  ...
  if (get_attestation_report(ias, config->apiver, b64quote, msg3->ps_sec_prop,
    ...
    cmac128(session->kdk, (unsigned char *)("\x01VK\x00\x80\x00"), 6,
            session->vk);
    ...
    if (msg4->status == Trusted) {
      ...
      cmac128(session->kdk, (unsigned char *)("\x01MK\x00\x80\x00"), 6,
              session->mk);
      cmac128(session->kdk, (unsigned char *)("\x01SK\x00\x80\x00"), 6,
              session->sk);
...
int get_sigrl(IAS_Connection *ias, int version, sgx_epid_group_id_t gid,
  ...
  while (1) {
    ...
    if (ret == IAS_UNAUTHORIZED &&
        (ias->getSubscriptionKeyID() ==
         IAS_Connection::SubscriptionKeyID::Primary)) {

      if (debug) {
        eprintf(
            "+++ IAS Primary Subscription Key failed with IAS_UNAUTHORIZED\n");
...
int get_attestation_report(IAS_Connection *ias, int version,
  ...
  if (status == IAS_OK) {
    ...
    } else if (!(reportObj["isvEnclaveQuoteStatus"].ToString().compare(
  ...
  case IAS_UNAVAILABLE:
    eprintf("Service is currently not able to process the request. Try again "
            "later.\n");
    break;
...
void usage() {
  cerr
      << "usage: sp [ options ] [ port ]" NL "Required:" NL
         "  -A, --ias-signing-cafile=FILE" NL
         "                           Specify the IAS Report Signing CA "
         "file." NNL "  -N, --mrsigner=HEXSTRING" NL
         "                           Specify the MRSIGNER value of encalves "
         "that" NL "                           are allowed to attest. Enclaves "
                   "signed by" NL
         "                           other signing keys are rejected." NNL
         "  -R, --isv-product-id=INT" NL
         "                           Specify the ISV Product Id for the "
         "service." NL "                           Only Enclaves built with "
                       "this Product Id" NL
         "                           will be accepted." NNL
         "  -V, --min-isv-svn=INT" NL "                           The minimum "
                                      "ISV SVN that the service provider" NL
         "                           will accept. Enclaves with a lower ISV "
         "SVN" NL "                           are rejected." NNL
         "Required (one of):" NL "  -S, --spid-file=FILE     Set the SPID from "
                                 "a file containg a 32-byte" NL
         "                           ASCII hex string." NNL
         "  -s, --spid=HEXSTRING     Set the SPID from a 32-byte ASCII hex "
         "string." NNL "Required (one of):" NL
         "  -I, --ias-pri-api-key-file=FILE" NL
         "                           Set the IAS Primary Subscription Key from "
         "a" NL "                           file containing a 32-byte ASCII "
                "hex string." NNL "  -i, --ias-pri-api-key=HEXSTRING" NL
         "                           Set the IAS Primary Subscription Key from "
         "a" NL "                           32-byte ASCII hex string." NNL
         "Required (one of):" NL "  -J, --ias-sec-api-key-file=FILE" NL
         "                           Set the IAS Secondary Subscription Key "
         "from a" NL "                           file containing a 32-byte "
                     "ASCII hex string." NNL
         "  -j, --ias-sec-api-key=HEXSTRING" NL
         "                           Set the IAS Secondary Subscription Key "
         "from a" NL "                           32-byte ASCII hex string." NNL
         "Optional:" NL "  -B, --ca-bundle-file=FILE" NL
         "                           Use the CA certificate bundle at FILE "
         "(default:" NL "                           "
      << DEFAULT_CA_BUNDLE
      << ")" NNL "  -D, --no-debug-enclave   Reject Debug-mode enclaves "
                 "(default: accept)" NNL
         "  -G, --list-agents        List available user agent names for "
         "--user-agent" NNL "  -K, --service-key-file=FILE" NL
         "                           The private key file for the service in "
         "PEM" NL "                           format (default: use hardcoded "
                  "key). The " NL "                           client must be "
                                  "given the corresponding public" NL
         "                           key. Can't combine with --key." NNL
         "  -P, --production         Query the production IAS server instead "
         "of dev." NNL
         ...
         "  -g, --user-agent=NAME    Use NAME as the user agent for contacting "
         "IAS." NNL "  -k, --key=HEXSTRING      The private key as a hex "
                    "string. See --key-file" NL
         "                           for notes. Can't combine with "
         "--key-file." NNL "  -l, --linkable           Request a linkable "
                           "quote (default: unlinkable)." NNL
         "  -p, --proxy=PROXYURL     Use the proxy server at PROXYURL when "
         "contacting" NL
         "                           IAS. Can't combine with --no-proxy" NNL
         "  -r, --api-version=N      Use version N of the IAS API (default: "
      << to_string(IAS_API_DEF_VERSION)
      << ")" NNL "  -v, --verbose            Be verbose. Print message "
                 "structure details and" NL
         "                           the results of intermediate operations to "
         "stderr." NNL "  -x, --no-proxy           Do not use a proxy (force a "
                       "direct connection), " NL
         "                           overriding environment." NNL
         "  -z  --stdio              Read from stdin and write to stdout "
         "instead of" NL
