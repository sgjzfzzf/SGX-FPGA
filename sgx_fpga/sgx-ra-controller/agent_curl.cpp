...
AgentCurl::~AgentCurl() { curl_easy_cleanup(curl); }
...
int AgentCurl::initialize() {
  ...
  // HTTPS only
  if (curl_easy_setopt(curl, CURLOPT_PROTOCOLS, CURLPROTO_HTTPS) != CURLE_OK)
    return 0;
...
int AgentCurl::request(string const &url, string const &postdata,
  ...
  if (postdata != "") {
    ...
    if ((slist = curl_slist_append(slist, "Content-Type: application/json")) ==
        NULL)
      return 0;
    ...
    if (curl_easy_setopt(curl, CURLOPT_POSTFIELDS, postdata.c_str()) !=
        CURLE_OK)
      return 0;
  }
...
static size_t _read_callback(char *buffer, size_t sz, size_t n,
                             void *instream) {
