...
#define LINE_TRAILING_LEN(header)                                              \
  ((LINE_MAX_LEN - string(header).size()) - LINE_SHORT_LEN - 2)

#define LINE_COMPLETE (string(LINE_MAX_LEN, LINE_TYPE).c_str())

#define LINE_HEADER(header)                                                    \
  (string(string(LINE_SHORT_LEN, LINE_TYPE) + ' ' + string(header) + ' ' +     \
          string(LINE_TRAILING_LEN(header), LINE_TYPE))                        \
       .c_str())
...
void divider(FILE *fd) { fprintf(fd, "%s\n", LINE_COMPLETE); }
