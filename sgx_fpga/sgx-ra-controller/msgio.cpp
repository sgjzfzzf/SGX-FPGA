...
MsgIO::MsgIO(const char *peer, const char *port) {
  ...
  if (peer == NULL)
    hints.ai_flags = AI_PASSIVE; // Server here
...
#endif
#ifdef IPV6_V6ONLY
      // If we have an IPV6 socket, make sure it will accept IPv4 connections,
      // too
      if (addr->ai_family == AF_INET6) {
        int disable = 0;
        setsockopt(s, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&disable,
                   sizeof(disable));
      }
#endif
...
int MsgIO::server_loop() {
  ...
  if (proto == AF_INET) {
    ...
    if (inet_ntop(proto, &sa->sin_addr, clihost, sizeof(clihost)) != NULL) {

      eprintf("%s", clihost);
    } else
      eprintf("(could not translate network address)");
  } else if (proto == AF_INET6) {
    char clihost[INET6_ADDRSTRLEN];

    memset(clihost, 0, sizeof(clihost));

    if (inet_ntop(proto, &cliaddr.sin6_addr, clihost, sizeof(clihost)) !=
        NULL) {
