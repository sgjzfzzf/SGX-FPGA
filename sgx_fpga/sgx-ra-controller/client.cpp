...
char certificate[351] =
    "W8+cDyhKnmqqA0RU/mu9xEouJFDwhrr69JI6KYthwbQ9aMppf/YQQbjGipMqR0PU\n"
...
int key_exchange_client(sgx_enclave_id_t src_enclave_id,
                        sgx_enclave_id_t dest_enclave_id, MsgIO *msgio);
...
uint32_t session_request_ocall(sgx_enclave_id_t src_enclave_id,
...
uint32_t exchange_report_ocall(sgx_enclave_id_t src_enclave_id,
                               sgx_enclave_id_t dest_enclave_id,
                               sgx_dh_msg2_t *dh_msg2, sgx_dh_msg3_t *dh_msg3,
                               uint32_t session_id) {
...
int main(int argc, char *argv[]) {
  ...
  if (config.mode == MODE_ATTEST) {
    ...
    while (msgio->server_loop()) {
      ...
      printf("The attestation time is: %ld\n",
             tv4.tv_sec * 1000 * 1000 + tv4.tv_nsec / 1000 -
                 (tv3.tv_sec * 1000 * 1000 + tv3.tv_nsec / 1000));
      ...
      if (result == 1) { // key exchange with client
        // key_exchange_client();
        // attest with FPGA
        timespec tv_11;
        ...
        bind(server_sockfd_adapter, (struct sockaddr *)&server_addr_adapter,
             sizeof(server_addr_adapter));
        listen(server_sockfd_adapter, 5);
        client_sockfd_adapter = accept(server_sockfd_adapter,
                                       (struct sockaddr *)&client_addr_adapter,
                                       &client_adapter_len);
        ...
        printf("VERIFY FPGA: %ld us\n",
               (tv_12.tv_sec * 1000 * 1000 + tv_12.tv_nsec / 1000) -
                   (tv_11.tv_sec * 1000 * 1000 + tv_11.tv_nsec / 1000));
        // fprintf(pFile, "%ld\n", (tv_12.tv_sec*1000*1000 + tv_12.tv_nsec/1000)
        // - (tv_11.tv_sec*1000*1000 + tv_11.tv_nsec/1000)); fclose(pFile);
  ...
  printf("The attestation time is: %ld\n",
         tv_2.tv_sec * 1000 * 1000 + tv_2.tv_nsec / 1000 -
             (tv_1.tv_sec * 1000 * 1000 + tv_1.tv_nsec / 1000));
...
int ctrl_process_data(sgx_enclave_id_t eid, int client_sockfd_adapter) {
  ...
  generate_encrypt_to_fpga_message(eid, &status_tmp, recv_data, data_size,
                                   send_data, data_size_output);
  ...
  generate_encrypt_to_app_message(eid, &status_tmp, fpga_message, receive_size,
                                  app_message, send_size);
  printf("%ld\n", (tv_test_2.tv_sec * 1000 * 1000 + tv_test_2.tv_nsec / 1000) -
                      (tv_1.tv_sec * 1000 * 1000 + tv_1.tv_nsec / 1000));
  // FILE *pFile_2;
  // pFile_2 = fopen("send_to_fpga_time.txt", "a");
  // fprintf(pFile_2, "%ld\n", (tv_test_2.tv_sec*1000*1000 +
  // tv_test_2.tv_nsec/1000)-(tv_test_1.tv_sec*1000*1000 +
  // tv_test_1.tv_nsec/1000)); fclose(pFile_2); for(int i = 0;i<send_size;i++)
  // eprintf("0x%x ", app_message[i]); eprintf("\n"); msgio->send((void **)
  // app_message, send_size);
