...
IAS_Connection::IAS_Connection(int server_idx, uint32_t flags,
                               char *priSubscriptionKey,
                               char *secSubscriptionKey) {
...
int IAS_Connection::setSubscriptionKey(SubscriptionKeyID id,
                                       char *subscriptionKeyPlainText) {
  ...
  if (subscriptionKeyPlainText == NULL ||
      (strlen(subscriptionKeyPlainText) != IAS_SUBSCRIPTION_KEY_SIZE) ||
  ...
  RAND_bytes((unsigned char *)subscription_key_xor[id],
             (int)sizeof(subscription_key_xor[id]));

  // XOR Subscription Key with One Time Pad to create an encrypted key
  for (int i = 0; i < IAS_SUBSCRIPTION_KEY_SIZE; i++)
    subscription_key_enc[id][i] = (unsigned char)subscriptionKeyPlainText[i] ^
                                  subscription_key_xor[id][i];
  ...
  if (debug && verbose) {
    ...
    eprintf(
        "+++ Encrypted Subscription Key[%d]:\t%s\n\n", id,
...
string IAS_Connection::getSubscriptionKey() {
  ...
  for (int i = 0; i < IAS_SUBSCRIPTION_KEY_SIZE; i++)
    keyBuff[i] = (subscription_key_enc[currentKeyID][i] ^
                  subscription_key_xor[currentKeyID][i]);
  ...
  if (debug) {
    ...
    eprintf("+++ One-time pad:\t\t\t%s\n",
            hexstring(subscription_key_xor[currentKeyID],
                      sizeof(subscription_key_xor[currentKeyID])));
    eprintf("+++ Encrypted SubscriptionKey:\t\t%s\n\n",
            hexstring(subscription_key_enc[currentKeyID],
                      sizeof(subscription_key_enc[currentKeyID])));
  }
