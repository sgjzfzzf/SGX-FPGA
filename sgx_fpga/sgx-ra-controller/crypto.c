...
int key_load(EVP_PKEY **pkey, const char *hexstring, int keytype) {
  ...
  if (keytype == KEY_PRIVATE) {
    ...
    if (!EC_POINT_mul(group, pubpt, gx, NULL, NULL, NULL)) {
...
unsigned char *key_shared_secret(EVP_PKEY *key, EVP_PKEY *peerkey,
                                 size_t *slen) {
...
int sha256_digest(const unsigned char *msg, size_t mlen,
...
int key_decrypt_certificate(uint8_t *msg, uint8_t *sig, size_t *slen,
                            EVP_PKEY *key) {
...
bool verifySignature(const char *publicKey, char *plainText,
