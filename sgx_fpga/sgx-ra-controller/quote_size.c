...
typedef sgx_status_t(SGXAPI *fp_sgx_get_quote_size_t)(const uint8_t *p_sig_rl,
                                                      uint32_t *p_quote_size);
typedef sgx_status_t(SGXAPI *fp_sgx_calc_quote_size_t)(const uint8_t *p_sig_rl,
                                                       uint32_t p_sigrl_size,
                                                       uint32_t *p_quote_size);
...
int get_quote_size(sgx_status_t *status, uint32_t *qsz) {
  ...
  fp_sgx_calc_quote_size = (fp_sgx_calc_quote_size_t)GetProcAddress(
      h_service, "sgx_calc_quote_size");
  if (fp_sgx_calc_quote_size == NULL) {
    // Then fall back to sgx_get_quote_size
    fp_sgx_get_quote_size = (fp_sgx_get_quote_size_t)GetProcAddress(
        h_service, "sgx_get_quote_size");
  ...
  *status = fp_sgx_calc_quote_size(NULL, 0, qsz);
  ...
  fp_sgx_calc_quote_size =
      (fp_sgx_calc_quote_size_t)get_sgx_ufunction("sgx_calc_quote_size");
  if (fp_sgx_calc_quote_size != NULL) {
    *status = (*fp_sgx_calc_quote_size)(NULL, 0, qsz);
    return 1;
  }

  fp_sgx_get_quote_size =
      (fp_sgx_get_quote_size_t)get_sgx_ufunction("sgx_get_quote_size");
  if (fp_sgx_get_quote_size == NULL)
    return 0;
